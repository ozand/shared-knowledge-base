# Claude Code: Complete Reference for All Practices and Approaches
## Beyond SKILLS, AGENTS, MEMORY, HOOKS: Comprehensive Guide
...
**CLAUDE.md** is the central project file that contains **persistent context** about the project. It's a "cheat sheet" for Claude that it reads before starting work.

```
KEY INSIGHT:
CLAUDE.md = Project's long-term memory
Claude ALWAYS reads it before working
Structures its understanding of the project
```
...
Structure:
.claude/
├── CLAUDE.md              ← Root context (entire system)
│
src/
├── CLAUDE.md              ← Local context (src/)
│
src/auth/
└── CLAUDE.md              ← Specific context (auth)

Rule: Claude reads CLAUDE.md in current directory
          + all parent CLAUDE.md files
          = accumulated context
```
...
### Switching Modes During Session
...
#### Example: /frontend:new-component command
...
**MCP** is a standard protocol for connecting Claude to external systems:
...
**Projects** is a collaborative feature in Claude (Pro/Teams/Enterprise) for creating isolated workspaces with shared context.
...
**@ referencing** is a way to explicitly tell Claude which files to include in context.

```
Syntax: @filename or @path/to/file
Result: Claude reads file and includes in context
Advantage: Faster than file-by-file manually
```
...
Claude reads both files automatically
...
**Coverage**: 12 key Claude Code practices
...
- [Complete Practices (Russian)](CLAUDE-COMPLETE-PRACTICES.md) - Original Russian version
